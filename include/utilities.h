...
int PIDCalculator(int limitPower, float input1, float input2, float desPoint, float kp, float ki, float kd){
  float error;  
  ...
  D_ = (error - past_error) * kd; //use error rate of change to make smooth transitions
  
  if(error == 0){I_ = 0;}         //when there is no error reset integral out
  
  output = P_ + I_ + D_;          //combine Outputs to make one output power
  
  if(output > limitPower){output = limitPower;}           //limit power to limitPower
  else if(output < -limitPower){output = -limitPower;}
